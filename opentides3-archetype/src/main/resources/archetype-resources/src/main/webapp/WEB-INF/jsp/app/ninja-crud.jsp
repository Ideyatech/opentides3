...
<div id="search-body">
	...
	<div id="results-panel" class="span9">
		...
		<div id="message-panel" class="row-fluid">
			...
			'>
				<app:status results="${symbol_dollar}{results}" />
			</div>
		</div>
		...
		<div class="table-wrapper-2 overflow-hidden">
			<div class="table-wrapper">	
				<table id="ninja-results" class="footable table-bordered table-striped table-hover table-condensed" data-page="${symbol_dollar}{results.currPage}" >
					<thead>
					...
					<tbody>
						<script type="text/template" class="template">
	                		<tr id="ninja-row-{{id}}" data-id="{{id}}">
								<td>
									<a href="${symbol_dollar}{home}/ninja/profile/{{id}}">
										{{completeName}}
									</a>
								</td>
								<td>
									<div class="btn-group">
									  <img class="img-polaroid" src="${symbol_dollar}{home}/ninja/photo?id={{id}}&size=xs"/>
									  <a class="btn dropdown-toggle" data-toggle="dropdown" href="${symbol_pound}"><span class="caret"></span></a>
									  <ul class="dropdown-menu">
									    <li><a data-url="${symbol_dollar}{home}/ninja/photo/upload?id={{id}}" class="upload-photo"><i class="icon-upload"></i> <spring:message code="photo.change-photo" /></a></li>
									    <li><a data-url="${symbol_dollar}{home}/ninja/photo/adjust?id={{id}}" class="adjust-photo"><i class="icon-edit"></i> <spring:message code="photo.edit-thumbnail" /></a></li>
									  </ul>
									</div>
								</td>
	                			<td>{{email}}</td>
						...
						<c:forEach items="${symbol_dollar}{results.results}" var="record" varStatus="status">
							<tr id="ninja-row-${symbol_dollar}{record.id}" data-id="${symbol_dollar}{record.id}">
								<td>
									<a href="${symbol_dollar}{home}/ninja/profile/${symbol_dollar}{record.id}">
										<c:out value="${symbol_dollar}{record.completeName}" />
									</a>
								</td>
								<td>
									<div class="btn-group">
									  <img class="img-polaroid" src="${symbol_dollar}{home}/ninja/photo?id=${symbol_dollar}{record.id}&size=xs"/>
									  <a class="btn dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></a>
									  <ul class="dropdown-menu">
									    <li><a data-url="${symbol_dollar}{home}/ninja/photo/upload?id=${symbol_dollar}{record.id}" class="upload-photo"><i class="icon-upload"></i> <spring:message code="photo.change-photo" /></a></li>
									    <li><a data-url="${symbol_dollar}{home}/ninja/photo/adjust?id=${symbol_dollar}{record.id}" class="adjust-photo"><i class="icon-edit"></i> <spring:message code="photo.edit-thumbnail" /></a></li>
									  </ul>
									</div>
								</td>
			                	<td><c:out value="${symbol_dollar}{record.email}" /></td>
			                	<td><c:out value="${symbol_dollar}{record.age}" /></td>
			                	<td><c:out value="${symbol_dollar}{record.score}" /></td>
			                	<td><fmt:formatDate value="${symbol_dollar}{record.joinDate}" pattern="MM/dd/yyyy"/></td>
			                	<td><c:out value="${symbol_dollar}{record.active}" /></td>
								<td>
									<i class='icon-pencil edit-action' data-id='${symbol_dollar}{record.id}' data-title="<spring:message code="label.edit" />"></i>
									<i class='icon-trash remove-action' data-id='${symbol_dollar}{record.id}' data-title="<spring:message code="label.delete" />"></i>
								</td>
							</tr>
						</c:forEach>
					</tbody>
		...
		<div class="paging clearfix">
			<app:paging results="${symbol_dollar}{results}"/>
		</div>
	</div>
...
<div id="form-body" class="modal fade hide">
	...
	<div id="form-panel" >
		<div class="modal-header">
			...
			<h4 class="${symbol_dollar}{add}"><spring:message code="label.ninja.add" /></h4>
			<h4 class="${symbol_dollar}{update}"><spring:message code="label.ninja.update" /></h4>
		</div>
		...
		<form:form modelAttribute="formCommand" id="ninja-form" cssClass="form-horizontal">
			<div class="modal-body" style="overflow-x: hidden !important">
				...
				<app:radio label="label.ninja.gender" path="gender" items="${symbol_dollar}{genderList}" itemLabel="value" itemValue="key" required="true"/>
				<app:select label="label.ninja.status" path="status" items="${symbol_dollar}{statusList}" itemLabel="value" itemValue="key" select2="true" />
				<app:select label="label.ninja.skills" path="skillSet" items="${symbol_dollar}{skillsList}" itemLabel="value" itemValue="key" multiple="true" select2="true" />
			...
			<div class="modal-footer">
				...
				<input type="submit" class="btn btn-info  ${symbol_dollar}{add}" data-form-display="add" data-submit="save-and-new" value="<spring:message code="label.save-and-new" />" />
...
<app:footer>
	<script type="text/javascript">
		${symbol_dollar}(document).ready(function() {
			${symbol_dollar}("${symbol_pound}ninja-body").RESTful();
			
			${symbol_dollar}('body').tooltip({selector: '.edit-action, .remove-action'});
			
			${symbol_dollar}('.footable').footable();
		})
