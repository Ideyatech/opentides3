...
if(aA.length<=aE+2){c.atmosphere.log("error",["Protocol data not sent by the server. If you enable protocol on client side, be sure to install JavascriptProtocol interceptor on server side.Also note that atmosphere-runtime 2.2+ should be used."])
}var ay=parseInt(c.trim(aA[aE+1]),10);
var aD=aA[aE+2];
...
}}catch(aB){}}}function ac(ay){i(ay,l);
i(ay,c.atmosphere)
